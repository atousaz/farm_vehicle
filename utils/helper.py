...
def clasify_segments(data, segments, spl):
    ...
    #M_best = pickle.load(open("C:/Users/atousaz/Box/saeed_data_distance/Data/finalized_model.sav", 'rb'))
    M_best = pickle.load(open("/Users/atousaz/Library/CloudStorage/Box-Box/saeed_data_distance/Data/finalized_model.sav", 'rb'))
...
#     avg_dist, poly_regression, poly_sumsquared_error, max_error=10, min_range=30,seq_range = [mk[0],mk[-1]])
    segments = slidingwindowsegment(
...
#   accelaration,velocity,vel_avg_l, acc_avg_l, l
...
#         print("seg", segment)
    ...
    vel2 = spl.derivative(n=1)(np.arange(len(data))/30)
    acc2=spl.derivative(n=2)(np.arange(len(data))/30)
    return  acc2,vel2,p,classes, segments, spl(np.arange(len(data))/30)
